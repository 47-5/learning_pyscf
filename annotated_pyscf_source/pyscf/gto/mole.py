...
class MoleBase(lib.StreamObject):
    ...
    Attributes:
        ...
        spin : int or None   # LRC NOTE: 这和自旋多重度不一样，比自旋多重度小1
        ...
        symmetry_subgroup : str
            subgroup  # LRC NOTE: symmetry_subgroup 用来把 topgroup 降到某个子群，只在 symmetry=True 自动检测时生效，因为有的太高的对称性反而不方便处理，认为降低对称性
        ...
        nucmod : dict or str or [function(nuc_charge, nucprop) => zeta]
            ...
            Default is point nuclear model.  # LRC NOTE: 原子核用什么模型？默认是点电荷
        nucprop : dict
            Nuclear properties (like g-factor 'g', quadrupole moments 'Q').
            It is needed by pyscf.prop module and submodules.
        cart : boolean
            Using Cartesian GTO basis and integrals (6d,10f,15g)  # LRC NOTE：是否使用笛卡尔型基函数
        magmom : list
        ...
        ** Following attributes are generated by :func:`Mole.build` **
        ...
        topgroup : str
            Point group of the system.  # LRC NOTE: topgroup 是检测到的最高点群
        groupname : str
            The supported subgroup of the point group. It can be one of SO3,
            Dooh, Coov, D2h, C2h, C2v, D2, Cs, Ci, C2, C1  # LRC NOTE: groupname 是实际用于计算的点群
        nelectron : int
            sum of nuclear charges - :attr:`Mole.charge` LRC NOTE: 总核电荷数-净电荷数目 = 总的电子数
        symm_orb : a list of numpy.ndarray  # LRC NOTE: 看我的symmetry_adapted_ao_basis.md
        ...
        irrep_id : a list of int  # LRC NOTE: 看我的symmetry_adapted_ao_basis.md
        ...
        irrep_name : a list of str  # LRC NOTE: 看我的symmetry_adapted_ao_basis.md
    ...
    # LRC NOTE: 这里使用了集合来存储键名，方便进行快速的成员检查，避免了使用列表时的线性查找效率问题。
    _keys = {
...
# don't modify the following private variables, they are not input options
        ...
        self._bas = numpy.zeros((0,8), dtype=numpy.int32)  # LRC NOTE: 这些形状和函数的注释里的描述能对上，但应该这只是其中一个元素
